...
void run_analysis()
{
    ...
    */
    ...
    //fopen_s(file_pointer, RESULT_FILE_NAME, "w");             /* Creates (opens) a result file using fopen_s */
...
void process_file(double array_to_populate[], FILE* pointer_to_data_file)
{
    ...
    while (fgets(line_buffer, LINESIZE, pointer_to_data_file)) {
        ...
        for (i = 0; i < values_per_line; i++) {
            ...
            array_to_populate[i + readings_processed] = extracted_values[i];
