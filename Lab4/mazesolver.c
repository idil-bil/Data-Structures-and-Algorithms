/*
 File:					mazesolver.c
 Purpose:				Mazesolver for Lab4 Take Home
...
void construct_shortest_path_info ( char** pathset, int numpaths, char* outputbuffer )
{
	...
	char* shortest_path = (char*)malloc(100 * sizeof(char));
	shortest_path = *pathset;

	for (count = 0; count < numpaths; count++) {
		if (strlen(*(pathset + count)) < length) {

			shortest_path = *(pathset + count);
			length = strlen(*(pathset + count));
		}
	}
...
void construct_cheapest_path_info ( char** pathset, int numpaths, char* outputbuffer )
{
	// INSERT CODE HERE
	int count = 0;
	int cost = path_cost(*pathset);
	char* cheapest_path = (char*)malloc(100 * sizeof(char));
	cheapest_path = *pathset;

	for (count = 0; count < numpaths; count++) { 
		if (path_cost(*(pathset + count)) < cost) { 
			cheapest_path = *(pathset + count); 
			cost = path_cost(*(pathset + count));
		}
	}
